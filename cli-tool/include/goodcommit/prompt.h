...
const static std::string SYSTEM_PROMPT =
    ...
    "- <UserPrompt> tag with the user's vague commit message (may be empty)\n"
    "- Tags named after each file path (e.g. <src/main.cpp>) containing that file's diff\n"
    "\n"
    "Analyze the diffs to understand what changed. If a <UserPrompt> is provided, use it as guidance. If empty, infer the commit type from the changes.\n"
    "\n"
    ...
    "- Under 72 characters\n"
    "- Base the message on the actual code changes in the diffs\n"
    "\n"
